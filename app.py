...
def submit():
    ...
    journal_sheet.append_row([timestamp, session["user"], title, entry])
...
def edit_entry(row):
    ...
    if request.method == "POST":
        ...
        entry = request.form["entry"]
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        journal_sheet.update(f"A{row}:D{row}", [[timestamp, session["user"], title, entry]])
        return redirect("/journal")
...
def profile():
    ...
    email = session["email"]  # ✅ now this will work
        ...
        for index, user in enumerate(users, start=2):  # start=2 because row 1 is header
            if user["Email"] == session["email"]:
                if check_password_hash(user["Password"], current_password):
                    ...
                    user_sheet.update_cell(index, 2, hashed)  # Assuming Email = Col 1, Password = Col 2
